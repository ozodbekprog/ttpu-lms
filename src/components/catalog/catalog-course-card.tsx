...
export function CatalogCourseCard({ course, staff }: { course: CatalogCourse; staff: boolean }) {
  return (
    <Card className="group flex h-full flex-col overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-200/80 hover:shadow-[0_2px_6px_rgba(16,24,40,0.05),0_16px_36px_-16px_rgba(29,52,96,0.22)]">
      ...
      <CardBody className="flex flex-1 flex-col">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            <h3 className="truncate font-semibold tracking-tight text-slate-900">
              <Link href={`/courses/${course.slug}`} className="transition-colors duration-150 hover:text-brand-700">
                {course.title}
              </Link>
            </h3>
        ...
        {course.description ? (
          <p className="mt-3 line-clamp-2 text-xs leading-relaxed text-slate-500">{course.description}</p>
        ) : null}
